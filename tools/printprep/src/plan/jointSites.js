...
export function placeJoints(soupA, soupB, plane, opts = {}) {
  ...
  const eps = 0.05;
  const secA = sectionOf(soupA, n, d + eps);
  const secB = sectionOf(soupB, n, d - eps);
  if (!secA.loops.length || !secB.loops.length) return null;

  const cell = Math.max(0.4, Math.sqrt(Math.max(secA.area, 1)) / 160);
  const gridA = rasterize(secA.loops, { cell, pad: 4 });
  if (!gridA.mask.length) return null;
  const gridB = rasterizeOnto(remapLoops(secB, secA), gridA);
  ...
  const S = quantize(Math.min(sMax, SQRT2 * (maxD - margin)), 0.5);
  if (S < sMin) return null;

  const P = params(S, opts.fit || {});
  const T = P.T;

  // Material-behind mask: solid at T/3, 2T/3 and T into both pieces.
  let solid = contact;
  for (const frac of [1 / 3, 2 / 3, 1]) {
    const sa = sectionOf(soupA, n, d + T * frac);
    const sb = sectionOf(soupB, n, d - T * frac);
    if (!sa.loops.length || !sb.loops.length) return null;
    solid = andMask(solid, { ...gridA, mask: rasterizeOnto(remapLoops(sa, secA), gridA).mask });
    solid = andMask(solid, { ...gridA, mask: rasterizeOnto(remapLoops(sb, secA), gridA).mask });
  }
